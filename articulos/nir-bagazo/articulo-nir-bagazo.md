...
```
                     ┌───────────────────────────────────────────────┐
                     │   MOLIENDA: del orden de 900 TCH / 21.000 TCD  │
                     │   ~960.000 ton/zafra de bagazo húmedo          │
                     └──────────────────────┬──────────────────────-─┘
                                            │
                              [ DESMEDULADO MECÁNICO ]
                                            │
            ┌───────────────────────────────┴───────────────────────────────┐
            ▼                                                               ▼
┌────────────────────────┐                                      ┌────────────────────────┐
│      FIBRA LARGA        │                                      │   MÉDULA + EXCEDENTE   │
│      (~45% - 50%)       │                                      │      (~50% - 55%)      │
└───────────┬────────────┘                                      └───────────┬────────────┘
            │                                                               │
            ▼                                                               ▼
┌────────────────────────┐                                      ┌────────────────────────┐
│ FÁBRICA DE PAPEL         │                                      │ CALDERAS DE ALTA        │
│ Digestor Pandia          │                                      │ PRESIÓN (45-65 bar)     │
│ ~130.000 ton/año papel   │                                      │ Cogeneración vapor/MW   │
└────────────────────────┘                                      └────────────────────────┘
```
...
```
                     ┌───────────────────────────────────────────────┐
                     │         INCREMENTO DE HUMEDAD EN BAGAZO        │
                     └───────┬───────────────┬───────────────┬───────┘
                             │               │               │
     ┌───────────────────────┘               │               └───────────────────────┐
     ▼                                       ▼                                       ▼
┌──────────────────────┐               ┌───────────────────────┐               ┌───────────────────────┐
│   CALDERAS (VAPOR)    │               │ MOLIENDA/EVAPORACIÓN  │               │   FÁBRICA DE PAPEL    │
├──────────────────────┤               ├───────────────────────┤               ├───────────────────────┤
│ • Caída de PCI         │               │ • Sobre-imbibición     │               │ • Variación licor/madera│
│ • Menor evap. específica│               │ • Dilución de Brix     │               │ • Dispersión de Kappa   │
│ • Quema de gas natural │               │ • Mayor consumo vapor  │               │ • Roturas de hoja        │
└──────────────────────┘               └───────────────────────┘               └───────────────────────┘
```
...
```
 00:00               00:15               00:30                               02:30         02:45         03:00
───┴───────────────────┴───────────────────┴───────────────────────────────────┴─────────────┴─────────────┴───►
 Muestreo            Picado y            Secado en estufa                    Enfriamiento  Cálculo y     DATO
 en cinta            preparación         tiro forzado                        y pesaje      comunicación  DISPONIBLE
...
## Calibración quimiométrica NIR online y control en tiempo real
...
```
                    ESPECTRÓMETRO NIR ONLINE (1.100 - 2.200 nm)
                    ┌────────────────────────────────────────┐
                    │   Bandas O-H: 1.450 y 1.940 nm (Agua)   │
                    │   Bandas C-H: 1.730 y 2.100 nm (Pol)    │
                    └───────────────────┬────────────────────┘
                                        │
                                        ▼
                       PROCESAMIENTO QUIMIOMÉTRICO (PLS)
                    ┌────────────────────────────────────────┐
                    │ • Normalización SNV                     │
                    │ • 1ra derivada Savitzky-Golay            │
                    │ • Filtro media móvil (60 s)               │
                    └───────────────────┬────────────────────┘
                                        │
                                        ▼
                          SISTEMA DCS / PANTALLA OPERADOR
                    ┌────────────────────────────────────────┐
                    │   Humedad: R² = 0,961 | RMSEP = ±0,38%  │
                    │   Pol:     R² = 0,944 | RMSEP = ±0,11%  │
...
Límite de humedad para combustión autosuficiente:
$$H(q_w, p_h) \leq H_{max}$$

Capacidad de evaporación en múltiple efecto:
$$V_{vapor,\,requerido}(q_w) \leq V_{vapor,\,capacidad}$$

Consistencia de calidad de pulpa celulósica:
$$\sigma_K\big(H(q_w,p_h)\big) \leq \sigma_{K,\,max}$$

Límites mecánicos y de extracción en molinos:
$$p_{h,\,min} \leq p_h \leq p_{h,\,max}$$

### Implementación computacional en VBA / Excel Solver

```vba
Option Explicit

Sub OptimizarTandemMolienda()
    ' Configuración de la optimización no lineal en Excel Solver
    SolverReset

    ' Celda objetivo: margen de contribución total del sistema -> Maximizar
    SolverOk SetCell:=Range("Margen_Total_Hora"), _
             MaxMinVal:=1, _
             ValueOf:=0, _
             ByChange:=Range("Caudal_Imbibicion, Presion_Hidraulica"), _
             Engine:=1, _
             EngineDesc:="GRG Nonlinear"

    ' Restricción 1: humedad de bagazo final <= umbral objetivo
    SolverAdd CellRef:=Range("Humedad_Bagazo_NIR"), _
              Relation:=1, _
              FormulaText:="0.500"

    ' Restricción 2: carga de vapor en evaporadores <= capacidad máxima
    SolverAdd CellRef:=Range("Demanda_Vapor_Evaporacion"), _
              Relation:=1, _
              FormulaText:=Range("Capacidad_Max_Vapor_Escape").Address

    ' Restricción 3: presión hidráulica en vírgenes de molinos (límites mecánicos)
    SolverAdd CellRef:=Range("Presion_Hidraulica"), Relation:=3, FormulaText:="2500"
    SolverAdd CellRef:=Range("Presion_Hidraulica"), Relation:=1, FormulaText:="3800"

    ' Restricción 4: ratio de imbibición sobre fibra
    SolverAdd CellRef:=Range("Ratio_Imbibicion_Fibra"), Relation:=3, FormulaText:="2.00"
    SolverAdd CellRef:=Range("Ratio_Imbibicion_Fibra"), Relation:=1, FormulaText:="3.00"

    ' Opciones del motor GRG Nonlinear
    SolverOptions Convergence:=0.0001, CentralDerivatives:=True, MultiStart:=True
...
El cuello de botella de la industria azucarera rara vez está en los fierros. Está en la latencia del dato. Cuando un proceso continuo se gestiona con información en tiempo real y rigor analítico, la fricción entre departamentos desaparece y el sistema converge solo hacia su punto de máxima eficiencia.
